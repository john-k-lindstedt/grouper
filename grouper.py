# todo:
# 
# - unify scene output and reading functions to use json exclusively.
#   format: {scene_id: {box_id: {x:###,y:###,w:###,h:###,l:###,r:###,b:###,t:###}}}
#   have features for inversion and otherwise. 
# 
# - track mean response / action times. 
#   want a sense of how confident / speedy / difficult each scene is
#   means will give a sense of activity per item, controlling for simple item-count effects
#
# - get working on slower machines, not sure if my current drawing solution will translate well. 
#
#
# - get some way to read in subject IDs -- 
# 
# - States:
# - init screen (takes subject ID)
# - instructions screen, uses scrollable text box with information
# - review state, when done with a trial.
#
# - need to develop a heads up display to give state feedback and input reminders.
...
class GroupingTask():
    ...
    final_text = pyglet.text.Label(text = "Task complete!", x = window.width//2, y = window.height//2,
                                  anchor_x = "center", anchor_y = "center",
                                  color = (0,0,0,255), font_size = 50)
    
    def __init__(self, window, scenes, subject_id = "test"):
        self.SID = subject_id
        ...
        self.evt_log = Logger(header = GroupingTask.event_log_header, logtype = "events")
        self.evt_log.open_log()
        
        self.trial_log = Logger(header = GroupingTask.trial_log_header, logtype = "trials")
        self.trial_log.open_log()
    ...
    def draw(self):
        ...
        if self.state == GroupingTask.STATE_COMPLETE:
            GroupingTask.final_text.draw()
...
class Scene():
    ...
    def __init__(self, scene_data, centered = True, flip_v = False, flip_h = False):
        self.boxes = []
        ...
        for sd in scene_data:
            x = sd[0] if not flip_h else (window.width - sd[0])
            y = sd[1] if not flip_v else (window.height - sd[1])
            w = sd[2]
            h = sd[3]
            
            self.l = min(self.l, x-(w//2))
            self.r = max(self.r, x+(w//2))
            self.b = min(self.b, y-(h//2))
            self.t = max(self.t, y+(h//2))
             
            self.boxes.append(Box(x,y,w,h, id = box_ID))
...
class Box():
    ...
    def __init__(self, x, y, w, h, id = -1):
        self.id = id
        self.x = x
        self.y = y
        self.w = w
    ...
    def clicked(self, mx, my, group):
        if mx <= self.r and mx >= self.l and my <= self.t and my >= self.b:
            ...
            return self.id if self.group != -1 else -1 * self.id
        else:
            return None
...
visicon = json.loads(open("sample_scene.json","r").read())
scene1 = Scene(scene_data = visicon, flip_v = True, flip_h = False)
scene2 = Scene(scene_data = visicon, flip_v = False, flip_h = False)
scene3 = Scene(scene_data = visicon, flip_v = True, flip_h = True)
scene4 = Scene(scene_data = visicon, flip_v = False, flip_h = True)
task = GroupingTask(window, scenes = [scene1, scene2, scene3, scene4])
